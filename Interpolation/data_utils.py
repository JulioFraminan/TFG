    ...
    for fname in sorted(os.listdir(folder)):
        ...
        try:
            with h5py.File(fpath, "r") as f:
                ...
                # Preferir ejes con la misma resolucion que el TL cargado
                X_raw = None
                Z_raw = None
        ...
        if roi_mode == "corner_fixed":
            ...
            # Ajustar esquina para que la ROI quepa dentro del plano
...
# ══════════════════════════════════════════════════════════════════════
#  INTERPOLACION LINEAL POR ANGULO
# ══════════════════════════════════════════════════════════════════════

def select_bracketing_indices(angles, target_angle):
    """Devuelve indices (low, high) y peso lineal para target_angle.

    Si target_angle cae fuera del rango, se usan los dos extremos mas cercanos
    y se clampa el peso a [0, 1].
    """
    angles_arr = np.asarray(angles, dtype=np.float32)
    if angles_arr.size == 0:
        raise ValueError("angles vacio: no hay planos disponibles.")
    ...
    exact = np.isclose(sorted_angles, target_angle, atol=1e-6)
    if np.any(exact):
        idx = int(sort_idx[int(np.where(exact)[0][0])])
        return idx, idx, 0.0, False

    pos = int(np.searchsorted(sorted_angles, target_angle))
    if pos <= 0:
        idx_low = int(sort_idx[0])
        idx_high = int(sort_idx[1])
    elif pos >= len(sorted_angles):
        idx_low = int(sort_idx[-2])
        idx_high = int(sort_idx[-1])
    else:
        idx_low = int(sort_idx[pos - 1])
        idx_high = int(sort_idx[pos])

    ang_low = float(angles_arr[idx_low])
    ang_high = float(angles_arr[idx_high])
    if ang_high == ang_low:
        weight = 0.0
    else:
        weight = (float(target_angle) - ang_low) / (ang_high - ang_low)
    ...
    return idx_low, idx_high, float(weight), clipped


def interpolate_planes(rois_norm, roi_angles, target_angle):
    """Interpola dos planos (normalizados) en funcion del angulo."""
    ...
    interp = (1.0 - weight) * plane_low + weight * plane_high
    return interp, idx_low, idx_high, weight, clipped


def load_interpolation_data(data_folder, roi_h, roi_w,
                            rois_per_plane,
                            roi_mode="center_max", roi_corner=(0, 0)):
    """Carga datos y normaliza para interpolacion lineal.

    Returns:
        (rois_norm, roi_angles, angles_norm, norm, roi_extents)
    """
    ...
    return rois_norm, roi_angles, angles_norm, norm, roi_extents
