...
def apply_spatial_smoothing(tl, sigma):
    """Aplica suavizado espacial (gaussiano) a un array TL."""
    return gaussian_filter(tl, sigma=sigma)
...
def _get_z_axis(Z_raw):
    ...
    Si Z_raw es (1, N) o compatible -> usa Z_raw[0, :]
    Si Z_raw es (N, 1) -> usa Z_raw[:, 0]
    Prioriza el eje que tenga > 1 elemento.
    """
    z_row = Z_raw[0, :].flatten()
    z_col = Z_raw[:, 0].flatten()
    # Elegir el que tiene más de 1 elemento; si ambos tienen >1, usar el más largo
    if z_row.size > 1 and z_col.size <= 1:
        return z_row
    elif z_col.size > 1 and z_row.size <= 1:
        return z_col
    else:
...
from config import (
    ...
    GAUSSIAN_SIGMA,
    USE_BLOCK_VARIABLES_WHEN_NO_SMOOTHING,
)
...
def _load_rois_from_folder(folder, roi_h, roi_w, rois_per_plane,
                            roi_mode, roi_corner, verbose_bounds=True, return_both=False):
    """Nucleo comun: carga .mat de *folder* y extrae ROIs.

    Si return_both=True, devuelve (rois, rois_block, angles, extents)
    donde 'rois' es la version tl suavizada o normal, y 'rois_block' es la version originaria tl_block.
    """
    all_rois, all_rois_block, all_angles, all_extents = [], [], [], []
    ...
    for fname in sorted(os.listdir(folder)):
        ...
        try:
            with h5py.File(fpath, "r") as f:
                ...
                # If `tl` is full-resolution (e.g. 2000x2000), using block axes
                # (e.g. 200x1) can force incorrect corner clamping.
                X_raw = None
                Z_raw = None
        ...
        if USE_GAUSSIAN_SMOOTHING:
            tl = apply_spatial_smoothing(tl, sigma=GAUSSIAN_SIGMA)
        ...
        if return_both:
            ...
            for roi, (oi, oj) in zip(plane_rois, plane_origins):
                plane_rois_block.append(tl_block[oi:oi + roi_h_px, oj:oj + roi_w_px])
        
        for idx, (roi, (oi, oj)) in enumerate(zip(plane_rois, plane_origins)):
            extent = compute_roi_extent(X_raw, Z_raw, oi, oj, roi_h_px, roi_w_px)
            all_rois.append(roi.astype(np.float32))
            if return_both:
                all_rois_block.append(plane_rois_block[idx].astype(np.float32))
...
def load_validation_rois(folder, roi_h, roi_w,
    ...
    res = _load_rois_from_folder(
        folder, roi_h, roi_w, 1,
        roi_mode, roi_corner, verbose_bounds=False, return_both=return_both,
    )
...
def load_inference_pipeline(model_path, data_folder, roi_h, roi_w,
    ...
    Returns:
        (device, model, rois_norm, rois_block_norm, roi_angles, angles_norm, norm, roi_extents)
    """
    ...
    rois, rois_block, roi_angles, roi_extents = load_all_rois(
    ...
    rois_block_norm = norm.normalize_tl(rois_block)
    ...
    return device, model, rois_norm, rois_block_norm, roi_angles, angles_norm, norm, roi_extents
