...
def main():
    ...
    with torch.no_grad():
        ...
        roi_angles_arr = np.sort(np.array(roi_angles))
        
        # Necesitamos reordenar base_samples segun el angulo
        sorted_idx = np.argsort(roi_angles)
        base_samples = base_samples[sorted_idx]

        for target_angle in GENERATE_ANGLES:
            # Buscar vecinos inferior y superior
            lower_mask = roi_angles_arr[roi_angles_arr <= target_angle]
            upper_mask = roi_angles_arr[roi_angles_arr >= target_angle]
            
            ang_lower = float(lower_mask[-1]) if len(lower_mask) > 0 else float(roi_angles_arr[-1])
            ang_upper = float(upper_mask[0]) if len(upper_mask) > 0 else float(roi_angles_arr[0])
            
            idx_lower = np.argmin(np.abs(roi_angles_arr - ang_lower))
            idx_upper = np.argmin(np.abs(roi_angles_arr - ang_upper))
            
            prev_roi = base_samples[idx_lower:idx_lower+1]
            next_roi = base_samples[idx_upper:idx_upper+1]
            
            seed = torch.cat([prev_roi, next_roi], dim=1)  # Tensor (1, 2, H, W)
            
            diff_next_prev = abs(ang_upper - ang_lower)
            diff_target_prev = abs(target_angle - ang_lower)
            ...
            idx = idx_lower
            seed_norm = rois_norm[sorted_idx[idx]]  # Use the original index mapping
            ...
            ext = roi_extents[sorted_idx[idx]]   # extension fisica de la semilla
            ...
            cond_seed = torch.tensor(
                [[0.0]], dtype=torch.float32, device=device, # 0.0 because it reconstructs prev_roi
            )
