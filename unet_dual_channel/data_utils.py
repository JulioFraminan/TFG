...
def generate_from_seed(model, seed_tensor, cond_val, norm, noise_std, device):
    """
    ...
    seed_tensor  : Tensor (1, 2, H, W) ya en device.
    cond_val     : float, factor de interpolación relativo `w`.
    norm         : Normalizer.
    noise_std    : float.
    device       : torch.device.
