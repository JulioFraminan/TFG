...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  3. PREPARAR DATASET BICANAL (prev, next) -> target y w
    # ══════════════════════════════════════════════════════════════════════
    # Ordenar por angulo
    sorted_idx = np.argsort(roi_angles)
    angles_sorted = np.array(roi_angles)[sorted_idx]
    rois_sorted = rois_norm[sorted_idx]
    ...
    for i in range(N):
        ...
        target_roi = rois_sorted[i]
        prev_roi = rois_sorted[prev_idx]
        next_roi = rois_sorted[next_idx]
        
        target_ang = angles_sorted[i]
        prev_ang = angles_sorted[prev_idx]
        next_ang = angles_sorted[next_idx]
        
        diff_next_prev = angle_diff(next_ang, prev_ang)
        diff_target_prev = angle_diff(target_ang, prev_ang)
        
        if diff_next_prev == 0:
            w = 0.5
        else:
            w = diff_target_prev / diff_next_prev
            
        bichans.append(np.stack([prev_roi, next_roi], axis=0))
        targets.append(np.expand_dims(target_roi, axis=0))
        ws.append(w)
        
    bichans = np.array(bichans, dtype=np.float32)
    targets = np.array(targets, dtype=np.float32)
    ws = np.array(ws, dtype=np.float32)

    # Si hay data augmentation (de forma simplificada lo ignoraremos para bicanal o se aplicaria al stack).
    # Como el prompt pide cambiar lo minimo, desactivamos augment o lo aplicamos a targets.
    # Para ser concisos, usaremos bichans y targets directamente.
    print(f"Dataset Bicanal generado: {len(bichans)} muestras (target intermedio)")
    
    idx_shuf = np.random.permutation(len(bichans))
    bichans_shuf = bichans[idx_shuf]
    targets_shuf = targets[idx_shuf]
    ws_shuf = ws[idx_shuf]

    tensor_inputs = torch.from_numpy(bichans_shuf)
    tensor_targets = torch.from_numpy(targets_shuf)
    tensor_ws = torch.from_numpy(ws_shuf[:, np.newaxis])
    
    use_pin = device.type == "cuda"
    loader = DataLoader(
        TensorDataset(tensor_inputs, tensor_ws, tensor_targets),
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_in, batch_w, batch_target in loader:
            # Mover batch a GPU (non-blocking si pin_memory=True)
            batch_in = batch_in.to(device, non_blocking=use_pin)
            batch_w = batch_w.to(device, non_blocking=use_pin)
            batch_target = batch_target.to(device, non_blocking=use_pin)
            known_mask = None

            if enable_train_inpaint:
                known_mask_np = generate_inpainting_known_masks(
                    batch_size=batch_target.shape[0],
                    height=batch_target.shape[2],
                    width=batch_target.shape[3],
                ...
                # Inpainting mode logic doesn't make much sense for 2-channel input if the target is another plane, 
                # but we'll apply the mask to the target and inputs if needed, or leave it for target since it reconstructs it.
                # Actually we won't apply holes to batch_in to avoid making the input have missing data, unless that's intended.
                # Just going to apply holes to target for loss calculation.
            ...
            if scaler is not None:
                with autocast("cuda"):
                    ...
                    if enable_train_inpaint:
                        ...
                        loss_holes = (torch.abs(recon - batch_target) * hole_mask).sum() / (hole_mask.sum() + 1e-8)
                        loss_known = (torch.abs(recon - batch_target) * known_mask).sum() / (known_mask.sum() + 1e-8)
                        loss = loss_holes + INPAINT_LOSS_KNOWN_WEIGHT * loss_known
                    else:
    ...
    with torch.no_grad():
        for page in range(n_pages):
            ...
            for c, j in enumerate(range(start, end)):
                # j es el indice en la lista ordenada
                sample_in = torch.from_numpy(
                    bichans[j][np.newaxis, :, :, :]
                ).to(device)
                w_val = ws[j]
                cond = torch.tensor(
                    [[w_val]], dtype=torch.float32, device=device
                )
                recon = model(sample_in, cond)
                recon_np = recon.cpu().squeeze().numpy()

                orig_tl = norm.denormalize_tl(rois_sorted[j])
                recon_tl = norm.denormalize_tl(np.clip(recon_np, 0, 1))
                errors = compute_error_metrics(orig_tl, recon_tl)
                angle_str = f"{angles_sorted[j]:.2f}"
                
                # recuperamos el extent original usando el indice en rois_norm.
                # pero wait, roi_extents estaba en rois_norm original.
                orig_index = sorted_idx[j]
                ext = roi_extents[orig_index]
                ...
                axes[0, c].set_title(f"Target -- {angle_str} (w={w_val:.2f})", fontsize=9)
        ...
        with torch.no_grad():
            for page in range(n_pages):
                ...
                for r, j in enumerate(range(start, end)):
                    ...
                    if len(lower_mask) > 0:
                        src_lower = float(lower_mask[-1])
                        ang_lower = src_lower
                    else:
                        src_lower = float(train_angles_sorted[-1])
                        ang_lower = src_lower - 360.0

                    if len(upper_mask) > 0:
                        src_upper = float(upper_mask[0])
                        ang_upper = src_upper
                    else:
                        src_upper = float(train_angles_sorted[0])
                        ang_upper = src_upper + 360.0

                    diff_lower = abs(circular_delta(val_angle, ang_lower))
                    diff_upper = abs(circular_delta(ang_upper, val_angle))
                    gap = abs(circular_delta(ang_upper, ang_lower))

                    # --- Generar predicción usando los vecinos circulares ---
                    idx_lower = np.argmin(np.abs(roi_angles_arr - src_lower))
                    idx_upper = np.argmin(np.abs(roi_angles_arr - src_upper))
                    
                    prev_roi = base_samples[idx_lower:idx_lower + 1]
                    next_roi = base_samples[idx_upper:idx_upper + 1]
                    # Tensor bicanal (B, 2, H, W)
                    seed = torch.cat([prev_roi, next_roi], dim=1)
                    
                    diff_next_prev = abs(circular_delta(ang_upper, ang_lower))
                    diff_target_prev = abs(circular_delta(val_angle, ang_lower))
                    
                    if diff_next_prev == 0:
                        w_val = 0.5
                    else:
                        w_val = diff_target_prev / diff_next_prev

                    gen_tl = generate_from_seed(
                        model, seed, w_val, norm, NOISE_STD, device,
                    )
                    ...
                    lo_str = f"{src_lower:+.2f}\u00b0 (dif={diff_lower:.2f}\u00b0)"
                    hi_str = f"{src_upper:+.2f}\u00b0 (dif={diff_upper:.2f}\u00b0)"
                    gap_str = f"{gap:.2f}\u00b0"
                    ...
                    neigh_txt = (
                        f"Vecino inf: {src_lower:+.1f}\u00b0 (\u0394={diff_lower:.1f}\u00b0)"
                        f"  |  Vecino sup: {src_upper:+.1f}\u00b0 (\u0394={diff_upper:.1f}\u00b0)"
                        f"  |  Gap total: {gap:.1f}\u00b0"
                    )
                    ...
                    axes[r, 1].set_title(
                        ...
                        f"(semillas {src_lower:.2f}\u00b0 / {src_upper:.2f}\u00b0)",
                        fontsize=8,
                    )
