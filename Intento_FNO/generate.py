...
def main():
    ...
    with torch.no_grad():
        ...
        for target_angle in GENERATE_ANGLES:
            ...
            axes[0].set_title(f"Generated -- target angle = {target_angle:.1f}", fontsize=11)
            ...
            axes[1].set_title(f"Seed original -- {roi_angles[idx]:.2f}", fontsize=11)
            ...
            axes[2].set_title(f"Seed reconstruction -- {roi_angles[idx]:.2f}", fontsize=11)
            ...
            fig.suptitle(
                f"Target angle: {target_angle:.1f}  |  Seed: {roi_angles[idx]:.2f}",
            ...
            plt.close(fig)
            print(
                f"  angle={target_angle:+7.1f}  (seed {roi_angles[idx]:.2f})"
                f"  MAE={mae_val:.2f} dB  Max={max_err:.2f} dB  ->  {mat_path}"
            )
