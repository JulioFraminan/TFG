...
def _discover_latest_checkpoint(results_folder: Path, min_modified_time: float | None = None) -> Path:
    candidates = []
    for path in results_folder.glob("model-*.pt"):
        if min_modified_time is not None and path.stat().st_mtime < min_modified_time:
            continue
    ...
    if len(candidates) == 0:
        ...
        raise FileNotFoundError(
            "No model-*.pt checkpoints created in this run. "
            "Avoid reusing old checkpoints by lowering save_and_sample_every "
            "or writing to a fresh results folder."
        )
...
def _next_checkpoint_milestone(results_folder: Path) -> int:
    max_milestone = 0
    for path in results_folder.glob("model-*.pt"):
        suffix = path.stem.split("-")[-1]
        if suffix.isdigit():
            max_milestone = max(max_milestone, int(suffix))
    return max_milestone + 1
...
def main() -> None:
    ...
    run_metadata_time = metadata_path.stat().st_mtime
    ...
    try:
        checkpoint_path = _discover_latest_checkpoint(results_folder, min_modified_time=run_metadata_time - 1e-3)
    except FileNotFoundError as error:
        print(f"[warn] {error}")
        print("[warn] Saving final checkpoint for current run before validation.")
        fallback_milestone = _next_checkpoint_milestone(results_folder)
        trainer.save(fallback_milestone)

        try:
            checkpoint_path = _discover_latest_checkpoint(results_folder, min_modified_time=run_metadata_time - 1e-3)
        except FileNotFoundError as second_error:
            print(f"Validation skipped: {second_error}")
            return
