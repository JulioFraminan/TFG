...
def normalize_to_neg1_1(x):
    """Normaliza array de [0, 1] a [-1, 1] para VAE.
    
    Parameters
    ----------
    x : np.ndarray en rango [0, 1]
    
    Returns
    -------
    x_vae : np.ndarray en rango [-1, 1]
    """
    return 2.0 * x - 1.0
...
def generate_from_seed(model, seed_tensor, target_angle, norm, noise_std, device):
    """
    Helper legado del pipeline UNet/FiLM.

    Parameters
    ----------
    model        : no usado en esta version.
    ...
    """
    raise RuntimeError(
        "generate_from_seed es un helper legado del pipeline UNet/FiLM. "
        "Para DiT usa generate.generate_samples_ddim(...)."
    )
...
def load_inference_pipeline(model_path, data_folder, roi_h, roi_w,
    ...
    Returns:
        (device, model, codec, rois_norm, roi_angles, angles_norm, norm, roi_extents)
    """
    from Transformers_2.No_usar.Intento_Transformers.model import DiT, LatentCodec
    from Transformers_2.No_usar.Intento_Transformers.config import verify_config
    ...
    latent_channels = int(checkpoint.get("latent_channels", 1))
    hidden_size = int(checkpoint.get("hidden_size", 128))
    depth = int(checkpoint.get("depth", 3))
    num_heads = int(checkpoint.get("num_heads", 2))
    mlp_ratio = float(checkpoint.get("mlp_ratio", 4.0))
    patch_size = int(checkpoint.get("patch_size", 4))
    attention_chunk_size = int(checkpoint.get("attention_chunk_size", 256))
    diffusion_steps = int(checkpoint.get("diffusion_steps", 1000))
    compression_ratio = int(checkpoint.get("compression_ratio", 8))
    ...
    return device, model, codec, rois_norm, roi_angles, angles_norm, norm, roi_extents
